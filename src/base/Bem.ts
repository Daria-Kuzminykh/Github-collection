...
export default class Bem {
    ...
    public block(modifiers: TModifiers = null) {
        return this._applyModifiers(this.blockName, modifiers);
    }

    public element(elementName: string, modifiers: TModifiers = null) {
        return this._applyModifiers(this.blockName + this.elementSeparator + elementName, modifiers);
    }

    private _applyModifiers(entity: string, modifiers: TModifiers) {
        const result = [];
        result.push(entity);
